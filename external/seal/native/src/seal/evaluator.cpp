...
namespace seal
{
    ...
    void Evaluator::multiply_inplace_hacked(Ciphertext &encrypted1, const Ciphertext &encrypted2,
        const std::vector<uint64_t> &mods1, const std::vector<uint64_t> &mods2,
        ...
        {
        case scheme_type::BFV:
            bfv_multiply_hacked(encrypted1, encrypted2, pool, mods1, mods2);
            break;
    ...
    void Evaluator::bfv_multiply_hacked(Ciphertext &encrypted1, const Ciphertext &encrypted2,
        MemoryPoolHandle pool, const std::vector<uint64_t> &mods1, const std::vector<uint64_t> &mods2) {
        if (encrypted1.is_ntt_form() || encrypted2.is_ntt_form())
        {
            throw invalid_argument("encrypted1 or encrypted2 cannot be in NTT form");
        }
        ...
        for (size_t i = 0; i < dest_count; i++)
        {
            for (size_t j = 0; j < coeff_mod_count; j++)
            {
                multiply_poly_scalar_coeffmod(
                    tmp_des_coeff_base.get() + (j * coeff_count) + (i * encrypted_ptr_increment),
                    coeff_count, mods1[j], coeff_modulus[j],
                    tmp_coeff_bsk_together_ptr + (j * coeff_count));
            }
            tmp_coeff_bsk_together_ptr += encrypted_ptr_increment;
            ...
            for (size_t k = 0; k < bsk_base_mod_count; k++)
            {
                multiply_poly_scalar_coeffmod(
                    tmp_des_bsk_base.get() + (k * coeff_count) + (i * encrypted_bsk_ptr_increment),
                    coeff_count, mods2[k], bsk_modulus[k],
                    tmp_coeff_bsk_together_ptr + (k * coeff_count));
            }
            tmp_coeff_bsk_together_ptr += encrypted_bsk_ptr_increment;
        }
    ...
    void Evaluator::multiply_monomial(Ciphertext &encrypted,
         size_t mono_exponent,  MemoryPoolHandle pool){
        auto &context_data = *context_->get_context_data(encrypted.parms_id());
        auto &parms = context_data.parms();
        auto &coeff_modulus = parms.coeff_modulus();
        size_t coeff_count = parms.poly_modulus_degree();
        ...
        for (size_t i = 0; i < encrypted_size; i++)
        {
            for (size_t j = 0; j < coeff_mod_count; j++)
            {
                negacyclic_multiply_poly_mono_coeffmod(
                    ...
                    1, mono_exponent, coeff_modulus[j],
    ...
    void Evaluator::multiply_plain_inplace_hacked(Ciphertext &encrypted,
        const Plaintext &plain, MemoryPoolHandle pool){
        ...
        auto plain_copy(allocate_zero_uint(coeff_count * coeff_mod_count, pool));
        // cout << "coeff count = " << coeff_count <<  ", coeff mod count = " << coeff_mod_count << endl;
        set_poly_poly(plain.data(), coeff_count, coeff_mod_count, plain_copy.get()); 
        ...
        for (size_t i = 0; i < encrypted_size; i++)
        {
            ...
            for (size_t j = 0; j < coeff_mod_count; j++, encrypted_ptr += coeff_count)
            {
                ...
                // Lazy reduction
                ntt_negacyclic_harvey_lazy(encrypted_ptr, coeff_small_ntt_tables[j]);
                dyadic_product_coeffmod(encrypted_ptr, plain_copy.get() + (j * coeff_count),
                    coeff_count, coeff_modulus[j], encrypted_ptr);
                inverse_ntt_negacyclic_harvey(encrypted_ptr, coeff_small_ntt_tables[j]);
            }
        }
        ...
        const GaloisKeys &galois_keys, MemoryPoolHandle pool)
    {
        ...
        {
	    cout << "Ignore: batching not supported" << endl;
                                    ...
                                    const GaloisKeys &galois_keys, MemoryPoolHandle pool)
    {
        ...
        for(size_t i = 0; i < steps.size(); i++)
        {
            ...
            if (steps[i] == 0)
            {
                ...
                galois_elt[i] = steps_to_galois_elt(steps[i], coeff_count);
            }
    ...
    // miran
    /**
     This function uses "util::apply_galois_ntt", which takes too long
     */
    void Evaluator::apply_galois_many(vector<Ciphertext> &encrypted, vector<uint64_t> galois_elt,
                                         const GaloisKeys &galois_keys, MemoryPoolHandle pool)
    {
        ...
        for(size_t r = 0; r < steps_size; r++)
        {
            ...
            if (kswitch_keys_index[r] >= kswitch_keys.data().size())
            {
                throw out_of_range("kswitch_keys_index");
            }
        }
        ...
        for (size_t i = 0; i < decomp_mod_count; i++)
        {
            ...
            for (size_t j = 0; j < rns_mod_count; j++)
            {
                ...
                for(size_t r = 0; r < steps_size; ++r)
                {
                    ...
                    // Calculate (temp * galois_key[0], temp * galois_key[1]) + (ct[0], 0)
                    // For each RNS decomposition, multiply with key data and sum up.
                    auto &key_vector = kswitch_keys.data()[kswitch_keys_index[r]];
        ...
        // Apply Galois to ct[r][0] in coefficient representation
        for(size_t r = 0; r < steps_size; r++)
        {
            for (size_t i = 0; i < coeff_mod_count; i++)
            {
