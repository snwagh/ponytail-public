...
namespace seal
{
    ...
    void Decryptor::decrypt_hacked(const Ciphertext &encrypted, std::vector<std::vector<uint64_t>> &result)
    {
        // Verify that encrypted is valid.
        if (!is_valid_for(encrypted, context_))
        {
        ...
        {
        case scheme_type::BFV:
            bfv_decrypt_hacked(encrypted, result, pool_);
            return;
    ...
    void Decryptor::bfv_decrypt_hacked(const Ciphertext &encrypted,
        std::vector<std::vector<uint64_t>> &result, MemoryPoolHandle pool)
    {
        if (encrypted.is_ntt_form())
        {
            throw invalid_argument("encrypted cannot be in NTT form");
	...
	result.resize(coeff_mod_count);
		...
		result[i][j] = tmp_dest_modq[j + (i * coeff_count)];
            }
        }
    }
